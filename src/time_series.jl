...
function plot!(t::TimeSeries{GaussianStateStochasticProcess}; label="", kwargs...)
    ...
    var_process = vcat([t[i].σ_t for i in 1:t.n_t]...)
    t_index = vcat([t[i].t for i in 1:t.n_t]...)
    plot!(t_index, mean_process - 1.96*sqrt.(var_process), fillrange = mean_process + 1.96*sqrt.(var_process), alpha=0.3, linealpha=0, label="CI 95 % $label"; kwargs...)
    plot!(t_index, mean_process, label="Mean $label"; kwargs...)
end
...
function plot(t::TimeSeries{ParticleSwarmState}; label="", index = 1:t.n_state, kwargs...)
    ...
    plot(t_index, q_low[:, index], fillrange = q_high[:, index], alpha=0.3, linealpha=0, label = hcat("CI 95 % ".*label...); kwargs...)
...
function plot!(t::TimeSeries{ParticleSwarmState}; label="", index = 1:t.n_state, kwargs...)
    ...
    if label == ""
        plot!(t_index, q_low[:, index], fillrange = q_high[:, index], alpha=0.3, linealpha=0; kwargs...)#, label = hcat("IC 95 % ".*label...))
        plot!(t_index, mean_process[:, index]; kwargs...)#, label = hcat("Mean ".*label...))
    else
        plot!(t_index, q_low[:, index], fillrange = q_high[:, index], alpha=0.3, linealpha=0, label = hcat("CI 95 % ".*label...); kwargs...)
        plot!(t_index, mean_process[:, index], label = hcat("Mean ".*label...); kwargs...)
    end
